...
from extronlib.system import ProgramLog, GetUnverifiedContext, Wait
...
#                self.authentication = None
        ...
        self.Commands = {
            ...
            'CurrentConferenceParameters': {'Parameters': ['Parameter'], 'Status': {}},
            'CameraMute': {'Status': {}},
            'Response': {'Status': {}},
            'Error': {'Status': {}},
            'SIP': {'Status': {}},
    ...
    def SetMicMute(self, value, qualifier):
        ...
        self.__SetHelper('MicMute', value, qualifier, url, data.encode())

    def SetReboot(self, value, qualifier):

        url = 'action.cgi?ActionID=WEB_RequestRebootAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        self.__SetHelper('Reboot', value, qualifier, url, data.encode())
    ...
    def UpdateMicMute(self, value, qualifier):
        ...
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('MicMute', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = ValueStateValues[temp['MicSwitch']]
                    self.WriteStatus('MicMute', value, qualifier)
                except (KeyError, IndexError):
                    self.Error(['MicMute: Invalid/unexpected response'])
    ...
    def UpdatePresentation(self, value, qualifier):
        ...
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('Presentation', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = ValueStateValues[temp['isSendAux']]
    ...
    def SetLogout(self, value, qualifier):
        ...
        url = 'action.cgi?ActionID=WEB_LogoutAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        self.__SetHelper('Logout', value, qualifier, url, data.encode())

    def UpdateAudioParams(self, value, qualifier):
        ...
        url = 'action.cgi?ActionID=WEB_InitAudioCtrlParamsAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        res = self.__UpdateHelper('AudioParams', value, qualifier, url, data.encode())
        if res:
            try:
                res['success'] == 1
            except KeyError:
                ...
                temp = loads(res['data'])
                try:
                    value = MuteStateValues[temp['MicSwitch']]
    ...
    def SetCameraPresetControl(self, value, qualifier):
        ...
        #data = dumps({"aszMsgIDs": ["MSG_CALL_RSP", "MSG_CALL_INCOMING"], "acCSRFToken": self.CSRFToken})
        #self.__SetHelper('SubscribeMessages', value, qualifier, url, data.encode())
...
######### MODIFIED
    ...
    def UpdateRemotePresentation(self, value, qualifier):
        ValueStateValues = {
            'auxClose': 'Not Receiving',
            'auxOpen': 'Receiving',
        }

        url = 'action.cgi?ActionID=WEB_IsReceiveRemAuxStrmAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('Presentation', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = ValueStateValues[temp['isSendAux']]
                    self.WriteStatus('RemotePresentation', value, qualifier)
                except (KeyError, IndexError):
                    self.Error(['Presentation: Invalid/unexpected response'])
    ...
    def SetPIP(self, value, qualifier):
        ...
        'local video' : 1,  
        'remote video': 2, 
        'local presentation': 3,
        'remote presentation': 4, 
        'pane not displayed': 5, 
        'pane in the upper left corner': 6, 
        'pane in the upper right corner': 7,
        'pane in the lower right corner': 8, 
        'pane in the lower left corner': 9,
        'auto PiP': 10, 
        '2 panes of the same size up and down': 11,
        '2 panes of the same size on left and right': 12,
        '2 panes of different sizes on left and right': 13,
        '3 panes one up and two down': 14, 
        '3 panes one left and two right': 15,
        'layout presented for all remote sites': 16,
        'remote video + local video': 17,
        'remote video + local video + conference presentation': 18,
        'remote video + conference presentation': 19
        }

        url = 'action.cgi?ActionID=WEB_ChangeCombinationMode'
        data = dumps({"Param1": ValueStateValues[value], "Param2": 0, "Param3": 0,"Param4": 0, "acCSRFToken": self.CSRFToken})
        self.__SetHelper('PIP', value, qualifier, url, data.encode())
        print('Set PIP {}'.format(value))

    def UpdateCurrentConferenceParameters(self, value, qualifier):
        url = 'action.cgi?ActionID=WEB_GetConfParamsInfoAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('CurrentConferenceParameters', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    remoteNumber = temp['remoteNumber']
                    if len(remoteNumber) > 0:
                        self.WriteStatus('CurrentConferenceParameters', remoteNumber, {'Parameter': 'Remote Number'})
                except (KeyError, IndexError):
                    self.Error(['CurrentConferenceParameters: Invalid/unexpected response'])
...
########### END MODIFIED
    ...
    def UpdateRequiredPolling(self, value=None, qualifier=None):
        #print('def UpdateRequiredPolling')
        if not self.Authentication:
            ...
            #print('Mailbox Sent')
            if res:
                #print('Mailbox Result: {}'.format(res))
                try:
                    if res['success'] == 1:
                        self.Authentication = True
                        ...
                        try:
                            ...
                            if ConferenceTypeStatusValues[temp['conftype']] == 'No call':
                                self.WriteStatus('CurrentConferenceParameters', '', {'Parameter': 'Remote Number'})
                        except (KeyError, IndexError):
        ...
        if self.RequiredPollingWait is None:
            self.RequiredPollingWait = Wait(4, self.UpdateRequiredPolling)
        else:
            self.RequiredPollingWait.Restart()
    ...
    def UpdateSessionID(self, value, qualifier):
        ...
        if self.RequiredPollingWait is None:
            self.RequiredPollingWait = Wait(5, self.UpdateRequiredPolling)
        else:
            self.RequiredPollingWait.Restart()
    ...
    def SetCameraMute(self, value, qualifier):
        ...
        url = ValueStateValues[value]
        data = dumps({"acCSRFToken": self.CSRFToken})
        self.__SetHelper('CameraMute', value, qualifier, url, data.encode())
    ...
    def UpdateCameraMute(self, value, qualifier):
        ...
        url = 'action.cgi?ActionID=WEB_IsSendBlueScreen'
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('CameraMute', value, qualifier, url, data.encode())
            if res:
                try:
                    #ProgramLog("res: {}".format(res), 'info')
                    temp = loads(res['data'])
                    #ProgramLog("temp: {}".format(temp), 'info')
                    value = ValueStateValues[temp['Param1']]
                    self.WriteStatus('CameraMute', value, qualifier)
                except (KeyError, IndexError):
                    self.Error(['CameraMute: Invalid/unexpected response'])
    ...
    def UpdateSIP(self, value, qualifier):
        ...
        url = 'action.cgi?ActionID=WEB_GetLineStateInfoAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('SIP', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = ValueStateValues[temp["sipStatusTxStr"]]
    ...
    def UpdateModel(self, value, qualifier):
        ...
        url = 'action.cgi?ActionID=WEB_GetVersionInfoAPI'
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('Model', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = temp["model"]
    ...
    def SetSIPAddress(self, value, qualifier):
        ...
        data = dumps({"CfgItemInt":[], "CfgItemString":[{"CfgItemID":ValueStateValues[value],
                "CfgItemInfo":"vcs-core-a.sber.ru"}],
                "acCSRFToken": self.CSRFToken})
        self.__SetHelper('SIPAddress', value, qualifier, url, data.encode())
    ...
    def UpdateSIPAddress(self, value, qualifier):
        ...
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('SIPAddress', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = temp["sipAddr"]
                    ProgramLog('SIPAddress Status value is: {}'.format(value), 'info')
                    self.WriteStatus('SIPAddress', value, qualifier)
                except KeyError:
                    self.Error(['SIPAddress Mode: Invalid/unexpected response'])
    ...
    def UpdateCamStatus(self, value, qualifier):
        ...
        url = 'action.cgi?ActionID=WEB_GetLocalCameraList'
        data = dumps({"acCSRFToken": self.CSRFToken})
        if self.Flag and self.Authentication:
            res = self.__UpdateHelper('CamStatus', value, qualifier, url, data.encode())
            if res:
                try:
                    temp = loads(res['data'])
                    value = ValueStateValues1[temp["itemList"][0]["itemState"]]
                    #value2 = ValueStateValues2[temp["itemList"][1]["itemState"]]
                    #value = value1 + value2
                    self.WriteStatus('CamStatus', value, qualifier)
                    ProgramLog('\n\n\nCAM {}, {}\n\n\n'.format(value, qualifier), 'info')
                except KeyError:
    ...
    def __CheckResponseForErrors(self, sourceCmdName, res):
        ...
        self.NewStatus('Response', res, None)
        return res
