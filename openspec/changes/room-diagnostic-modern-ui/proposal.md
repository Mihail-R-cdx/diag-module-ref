...
- **Device-family visual redesigns:** the common room shell and accordion foundation are implemented here, but detailed expanded-screen redesigns are not. Existing supported Audio DSP, Matrix/IN1804, codec, and PDU presentations remain the functional content used inside the common accordion. Their dedicated modern visual redesigns will be separate follow-up OpenSpec changes after this foundation reaches `master`.
...
- Integrate the existing supported expanded device presentations into the common room accordion without redesigning their family-specific internal layout in this change. Existing Matrix routing, PDU mutation/reconciliation, codec auxiliary/live behavior, Audio DSP live metering, and every exact-row lifecycle boundary remain owned by their existing application/controller paths.
- Do not introduce new device-family reference-only controls merely to match the deferred screen redesigns. Any future gain/mute/reboot/card/table/meter redesign belongs to the later device-family change that owns that screen.
...
operator types target query
    ...
    -> room mode
        ...
        -> existing device-family expanded presentations
...
- `diagnostic-ui-presentation`: self-contained common room-shell visual contract, semantic themes, selected-room cue, room/network cards, call-derived busy presentation, switch-parent port summary, common equipment-row contract, and compatibility boundary for existing expanded device presentations. Detailed family-specific expanded-screen redesign is explicitly outside this capability's current change scope.
