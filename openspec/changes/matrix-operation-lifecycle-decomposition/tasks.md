...
- [x] 11.10 Detach failed same-identity persistent sessions while the Matrix serialized operation lane is still held so a newer same-identity operation cannot reuse the failed handler; covered by `test_failed_session_invalidation_is_serialized_for_same_identity_race`.
- [x] 11.11 Propagate real IN1804 production command transport/session failures as structured controller failures instead of accepting synthetic default full-refresh data; covered by `test_production_full_refresh_transport_failure_is_not_accepted_or_cached` and `test_production_full_status_transport_failure_is_not_default_success`.
- [x] 11.12 Prevent quick refresh from publishing fabricated route 1 after a broken production session; covered by `test_production_quick_refresh_transport_failure_does_not_emit_route_one` and `test_production_get_connections_transport_failure_is_structured`.
- [x] 11.13 Reconnect with a fresh handler after a production full-refresh session failure and keep failed refresh results out of credential-success memory; covered by `test_production_full_refresh_transport_failure_is_not_accepted_or_cached`.
- [x] 11.14 Treat post-send route authentication prompts as unknown state-changing outcomes without replaying the route command or starting credential fallback; covered by `test_post_send_auth_prompt_does_not_replay_route_command` and `test_route_auth_after_possible_send_is_not_retried`.
- [x] 11.15 Treat empty state-changing Matrix command responses after possible send as unknown outcomes, not success or replay authority; covered by `test_route_empty_response_is_unknown_outcome_and_not_success` and `test_route_empty_response_does_not_accept_or_fallback`.
- [x] 11.16 Treat empty read-only Matrix command responses as structured non-success outcomes that cannot fabricate route/status/default full-refresh data; covered by `test_read_only_empty_response_is_not_success` and `test_full_refresh_empty_response_is_not_accepted_and_reconnects_next`.
- [x] 11.17 Bound read-only Matrix re-authentication recovery to one automatic re-authentication plus one replay using the same assigned credential; covered by `test_read_only_auth_recovery_is_bounded` and `test_read_only_auth_recovery_succeeds_within_budget`.
- [x] 11.18 Keep route credential fallback impossible after empty or otherwise ambiguous post-send outcomes; covered by `test_route_empty_response_does_not_accept_or_fallback`.
- [x] 11.19 Reject echo-only Matrix command responses after command echo normalization when authoritative response is required, while preserving echo plus payload success; covered by `test_route_echo_only_response_is_unknown_outcome`, `test_read_only_echo_only_response_is_not_success`, `test_read_only_echo_plus_payload_is_success`, `test_route_echo_only_response_does_not_accept_or_fallback`, and `test_full_refresh_echo_only_response_is_not_accepted`.
