...
PDU outlet control operations for Extron IPL T PCS4i SHALL execute outside the
Qt GUI thread through a background PDU command worker. If the existing Aten
PE8208AV command path is touched by this change, it SHALL be moved to the same
background command boundary without changing Aten protocol semantics.
...
- **WHEN** the implementation changes Aten outlet command dispatch as part of the PDU background boundary
- **THEN** Aten ON, OFF, and REBOOT commands run through the background PDU command worker and retain existing handler behavior
...
PDU refresh and command workers SHALL carry a non-secret operation generation
for the selected model, IP address, and credential context. A queued operation
SHALL recheck that generation immediately before handler acquisition and
network I/O. A stale queued operation SHALL be dropped without creating a
handler, opening Telnet or HTTP transport, or sending a command.
...
ambiguous outcome; REBOOT SHALL not be automatically replayed when first
