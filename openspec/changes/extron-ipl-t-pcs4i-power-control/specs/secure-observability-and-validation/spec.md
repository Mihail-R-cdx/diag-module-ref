...
live PCS4i hardware, and hardware-mutating tools SHALL remain opt-in.

#### Scenario: Telnet authentication cases are synthetic
- **WHEN** maintainers run the offline PCS4i authentication tests
- **THEN** the tests cover prompt chunking, two-send limits, success, rejection, timeout, and disconnect using synthetic transport data

#### Scenario: PDU command safety is offline-testable
- **WHEN** maintainers run offline PDU command tests
- **THEN** ambiguous ON/OFF reconciliation and ambiguous REBOOT non-replay are verified without sending commands to live hardware
