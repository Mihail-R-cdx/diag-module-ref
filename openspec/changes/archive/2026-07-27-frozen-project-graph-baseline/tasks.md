...
- [x] Add `.gitattributes` with exact LF pins for `.graphifyignore` and all four allowlisted generated artifacts so Windows `core.autocrlf=true` checkouts preserve committed byte hashes.
...
- [x] Implement `tools/refresh_project_graph.ps1` with explicit initial, incremental, full-rebuild, opt-in exact-install modes, clean `SourceRoot`/`OutputRoot` separation, required `BaselineStage`/`SourceRef`/`TargetBranch`, canonical UTF-8-without-BOM/LF generated artifact publication, schema-v2 metadata, and no dirty publish mode.
...
- [x] Create `graphify-out/baseline.json` with full indexed source SHA, exact version, hashes over actual canonical published bytes, counts, and no local identity/path data.
...
- [x] Repeat build or compare topology on the same source commit and verify identical source-file set, node identities, edge identities, counts, committed `.graphifyignore` byte hash, and canonical generated artifact byte policy.
