...
- [x] 4.8 Historical behavior allowed XTP/XTP II naming and DTP/XTP/XTP II temperature to remain unavailable when evidence was absent. The naming portion remains valid only where the approved deterministic GUI fallback applies; the temperature portion is superseded by 26.12-26.15 and is no longer an acceptable implementation or hardware-PASS outcome.
...
- [ ] 26.12 **PRE-IMPLEMENTATION ARCHITECTURE GATE:** close every `REQUIRED-PROVE (blocking)` cell in the General-information acquisition matrix with an authoritative exact-profile source, exact command/endpoint, accepted response grammar, and evidence reference. This is architecture/protocol research, not implementation work; no production implementation session may start while any blocking cell remains. MAC/serial may use canonical inventory first, but their absent-inventory fallback still requires a proven device source.
- [ ] 26.13 **After 26.12 is architecture-reviewed with zero blocking cells**, implement normalized Matrix full-refresh state and completion gating so accepted complete success requires authoritative model, MAC, serial, firmware, temperature, and uptime; any missing required value produces an incomplete/failed current result and clears stale presentation evidence.
- [ ] 26.14 Add focused regressions for every supported Matrix profile proving successful full refresh supplies all six General-information values, missing inventory MAC/serial uses only the proven exact-profile fallback, missing/unproven required fields prevent complete success, and no placeholder/stale/synthetic value satisfies the gate.
- [ ] 26.15 Reconcile `hardware-validation.md` and hardware evidence against the six-field mandatory gate and the approved acquisition matrix; a hardware run with any General-information `Нет данных` or any unresolved acquisition cell remains PARTIAL/FAIL/BLOCKED as applicable, never PASS.
