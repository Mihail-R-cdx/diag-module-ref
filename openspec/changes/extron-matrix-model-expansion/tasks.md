...
- [ ] 26.12 Establish authoritative per-profile acquisition evidence for all six required General-information values across every in-scope Matrix model. MAC/serial may use current canonical inventory evidence first but require a proven device fallback when missing; firmware, temperature, and uptime require authoritative device reads. Do not implement guessed cross-family commands.
- [ ] 26.13 Extend normalized Matrix full-refresh state and completion gating so accepted complete success requires authoritative model, MAC, serial, firmware, temperature, and uptime; any missing required value produces an incomplete/failed current result and clears stale presentation evidence.
...
- [ ] 26.15 Reconcile `hardware-validation.md` and hardware evidence against the six-field mandatory gate; a hardware run with any General-information `Нет данных` remains PARTIAL/FAIL/BLOCKED as applicable, never PASS.
