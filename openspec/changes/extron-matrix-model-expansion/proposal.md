...
- Keep CrossPoint route semantics family-specific. For approved DTP CrossPoint profiles, the canonical `routes[output_id]` state is the video tie only: read `<O>%`, set `<I>*<O>%`, and untie `0*<O>%`. Audio ties are outside the current Matrix table and SHALL NOT be changed by a DTP route intent. XTP/XTP II keep their separately approved AV-route syntax and SHALL NOT inherit DTP `%` semantics by family similarity.
