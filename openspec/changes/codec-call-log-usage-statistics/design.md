...
5. Stop retrieval early once the 90-day calculation boundary is covered or end-of-journal is reached.
6. Degrade transparently when the 100-record ceiling prevents a requested period from being covered.
7. Keep literal device call-log semantics, including failed/unanswered records and legitimate overlapping records.
8. Keep active calls visible but out of completed-usage totals.
9. Preserve Huawei and Polycom session/worker ownership, typed failures, stale suppression, credential authority, and background-only network I/O.
10. Make real-device protocol validation useful without requiring devices to contain artificial 100-call or 90-day histories.
...
- Do not interpret a quiet period as unavailable history when the protocol has returned end-of-journal.
...
source_ended               whether the protocol reported no older entries
limit_reached              whether the product ceiling of 100 accepted records stopped retrieval
warnings[]                 non-secret structured completeness/parse/time warnings
```

The exact Python shape is an implementation choice, but arithmetic fields SHALL remain machine-readable until presentation formatting.
...
## Decision 3: Deep retrieval is newest-first and hard-bounded at 100 records
...
1. enough history has been obtained to prove coverage past the start of the 90-day calendar window;
2. the protocol reports end-of-journal/no more records;
3. 100 accepted call-log records have been obtained;
4. a typed operational failure prevents further retrieval.

No implementation path may fetch a 101st accepted call-log record for this dialog load merely to improve statistics.

If a fetched page contains entries on both sides of the 90-day boundary, the implementation MAY retain the boundary-crossing/older entry necessary to establish coverage but SHALL NOT issue another history request after coverage is proven. Records older than the longest calculation interval need not be kept for GUI presentation or totals except as needed to establish interval overlap/coverage.
...
## Decision 4: Vendor protocol details are established before implementation is locked

The common architecture deliberately does not invent undocumented vendor pagination or device-time parameters. Before changing each model-specific retrieval path, the implementation session SHALL inspect current source and, where source/documented fixtures are insufficient, perform read-only live research against the available device family to establish:
...
- exact end-of-journal indication;
...
A live device with only a handful of recent records is sufficient to validate real request/response, ordering, next-page/end behavior, and time encoding that are observable. Synthetic fixtures/fakes SHALL provide deterministic proof for unavailable live edge cases such as 100+ records and 90-day history.

## Decision 5: Calendar authority is codec time with explicit system-time fallback
...
## Decision 6: Usage numerator follows literal completed call-log duration and interval overlap
...
## Decision 7: Working-capacity denominator is weekday-count times eight hours
...
- In a degraded interval beginning at an exact oldest-record timestamp partway through a weekday, that touched weekday still contributes the full eight hours.
...
## Decision 8: The 100-record ceiling has explicit degradation semantics

The normal desired summaries are 30 days and 90 days. Degradation occurs because of the product-imposed 100-record ceiling, not merely because a clean source end occurs early.

When the 100th accepted record is reached before a target period is covered, the degraded interval begins at the exact `start_at` timestamp of the oldest accepted record and ends at `reference_now`.

Its label day count is the number of calendar dates touched inclusively from `oldest.start_at.date()` through `reference_now.date()`. Arithmetic uses the exact timestamp interval, while the denominator still counts every touched Monday-Friday calendar date as a full eight-hour day.

Presentation rules are:

- If 100 records cover the full 90-day window, show normal 30-day and 90-day rows.
- If 100 records cover at least the full 30-day window but not the 90-day window, show one normal 30-day row plus one degraded row labelled with the actual covered calendar-day count (for example 47 days). The degraded row recalculates both numerator and denominator for its actual interval; it is not a 90-day percentage with missing numerator data.
- If 100 records do not cover the full 30-day window, show exactly one degraded row for the actual covered interval rather than two duplicate rows derived from the same history.
- Every degraded row SHALL carry a clear warning that the calculation depth was limited by the 100-record ceiling.

If a typed retrieval failure occurs before a target window is proven complete, a normal-looking full-period percentage SHALL NOT be manufactured. Any shorter target already fully covered remains valid; the incomplete longer target is marked unavailable/incomplete with an error warning. Visible records already obtained remain usable in the journal.

## Decision 9: Clean source end and empty history are valid complete data

If the codec reports end-of-journal before 30 or 90 calendar days are reached, the application treats all older time in those requested windows as zero call usage. Both requested windows remain complete under the product's simplified source-end rule.
...
## Decision 10: The GUI has two loading stages and partial-result isolation
...
A deep-history failure SHALL NOT discard already accepted recent records. If 30-day coverage had already been proven before a later failure, the valid 30-day row remains visible while the longer incomplete calculation is warned/unavailable.

Warnings for system-time fallback, 100-record degradation, malformed durations, or incomplete retrieval SHALL be non-secret and non-modal. They SHALL NOT convert otherwise usable recent call history into a modal operation failure.

## Decision 11: Existing Huawei and Polycom lifecycle ownership is preserved
...
## Decision 12: Testing separates protocol reality from deterministic algorithm coverage
...
- current weekday and partial first degraded weekday both contributing full eight hours;
...
- early stop after proving 90-day coverage;
- hard stop at exactly 100 accepted records with no 101st accepted record;
- 100-record coverage shorter than 30 days producing one degraded row;
- 100-record coverage between 30 and 90 days producing one full 30-day row plus one degraded longer row;
- a deep retrieval failure preserving already loaded journal and any already proven shorter statistic;
...
Live validation SHALL be observational and shall confirm the real protocol facts that are actually observable on available devices. It is not required to manufacture 100 calls, old calls, active calls, malformed records, or 90 days of history. Missing live edge cases SHALL remain covered by deterministic synthetic tests rather than being waived.
